...
class Rol(Base):
    ...
    permisos_rol: Mapped[list["PermisoRol"]] = relationship(
        "PermisoRol", back_populates="rol")
...
class Usuario(Base):
    ...
    id_persona: Mapped[int] = mapped_column(
        ForeignKey("persona.id_persona"), nullable=False)
...
class Auditoria(Base):
    ...
    accion: Mapped[AccionAuditoriaEnum] = mapped_column(
        Enum(AccionAuditoriaEnum), nullable=False)
    id_usuario: Mapped[int] = mapped_column(
...
class Operacion(Base):
    ...
    id_operacion: Mapped[int] = mapped_column(
        Integer, primary_key=True, autoincrement=True)
    nombre: Mapped[str] = mapped_column(String(45), nullable=False)
...
class Permiso(Base):
    ...
    __table_args__ = (
        UniqueConstraint("id_modulo", "id_operacion",
                         name="UK_modulo_operacion"),
    )
    ...
    id_operacion: Mapped[int] = mapped_column(
        ForeignKey("operacion.id_operacion"), nullable=False)
...
class PermisoRol(Base):
    ...
    id_rol: Mapped[int] = mapped_column(
        ForeignKey("rol.id_rol"), primary_key=True)
    id_permiso: Mapped[int] = mapped_column(
