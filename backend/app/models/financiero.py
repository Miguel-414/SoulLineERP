...
class Proveedor(Base):
    ...
    facturas: Mapped[list["Factura"]] = relationship(
        "Factura", back_populates="proveedor")
...
class Factura(Base):
    ...
    id_proveedor: Mapped[int] = mapped_column(
        ForeignKey("proveedor.id_proveedor"), nullable=False)
    total_bruto: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=0)
    iva: Mapped[Decimal | None] = mapped_column(Numeric(5, 2), default=0)
    valor_iva: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 2), default=0)
    retencion_fuente: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 2), default=0)
    descuento: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 2), default=0)
    valor_total: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False, default=0)

    proveedor: Mapped["Proveedor"] = relationship(
        "Proveedor", back_populates="facturas")
...
class DetalleFactura(Base):
    ...
    id_detalle_factura: Mapped[int] = mapped_column(
        Integer, primary_key=True, autoincrement=True)
    id_factura: Mapped[int] = mapped_column(
        ForeignKey("factura.id_factura"), nullable=False)
    ...
    id_item_serializado: Mapped[int | None] = mapped_column(
        ForeignKey("item_serializado.id_item_serializado"), unique=True)
    ...
    valor_bruto: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False)
    tasa_impuesto: Mapped[Decimal] = mapped_column(
        Numeric(5, 2), nullable=False, default=0)
    valor_impto_cargo: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), nullable=False)
    ...
    factura: Mapped["Factura"] = relationship(
        "Factura", back_populates="detalles")
